...
add_contribution <- function(tree, contribution = NULL, signatures = NULL, mut_matrix = NULL, ...) {
  ...
  if(!is.null(contribution)) {
    ...
    add = rbind(add, as.data.frame(matrix(0, nrow = length(branches_missing), ncol = ncol(add), dimnames = list(branches_missing, colnames(add))))) %>% 
...
plot_gg_tree <- function(tree, common_name = NA, add_x = TRUE, 
  ...
  # add any info
  if (add_branch_id) gg = gg + geom_text(aes(x=branch, label=branch_id), nudge_y = 0.5, size = 3, na.rm = T)
  if (add_branch_length) gg = gg + geom_text(aes(x=branch, label=branch_length), nudge_y = -0.5, size = 2, na.rm = T)
